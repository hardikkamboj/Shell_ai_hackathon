...
def getTurbLoc(turb_loc_file_name):
    ...
    turb_coords = np.array(turb_loc_file_name)
    return(turb_coords)
...
def preProcessing(power_curve,num_turbines):
    """
    -**-THIS FUNCTION SHOULD NOT BE MODIFIED-**-
...
def find_aep(turbine_data_file,power_curve,wind_data,debug = False):
    ...
    turb_coords   =  getTurbLoc(turbine_data_file)
    
    # Load the power curve
    power_curve   =  loadPowerCurve(power_curve)
    ...
    n_wind_instances, cos_dir, sin_dir, wind_sped_stacked, C_t = preProcessing(power_curve,num_turbines = turb_coords.shape[0])
    if debug:
    	print(n_wind_instances, cos_dir, sin_dir, wind_sped_stacked, C_t)
