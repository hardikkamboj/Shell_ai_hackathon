...
def totalAEP(turb_diam, turb_coords, power_curve, wind_inst_freq,debug = False):
    ...
    for i in tqdm(range(n_slices_drct), disable=False):
        for j in range(n_slices_sped): 
            ...
            pwr,part_turb_pwr  = partAEP(n_turbs, turb_diam, turb_coords, 
                           power_curve, wind_drct, wind_sped,debug = debug)

            #################################################################################
            # added by me
            total_turb_pwr.append(part_turb_pwr)
    ...
    AEP = AEP/1e3
    if debug:
        return(AEP,farm_pwr_arr,total_turb_pwr)
    else:
        return (AEP)    
