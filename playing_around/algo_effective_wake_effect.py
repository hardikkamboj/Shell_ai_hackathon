...
def jensenParkWake(n_turbs, turb_diam, rotate_coords, power_curve, wind_sped):
    ...
    return(sped_deficit,impact_on_ibyj)


def partAEP(n_turbs, turb_diam, turb_coords, power_curve, wind_drct, wind_sped,debug = False):
    """
    -**-THIS FUNCTION SHOULD NOT BE MODIFIED-**-
    ...
    sped_deficit,impact_on_ibyj = jensenParkWake(n_turbs, turb_diam, rotate_coords, power_curve, wind_sped)
    ...
    ##############################################################################
    # print(type(impact_on_ibyj))
    # print(impact_on_ibyj.shape)
    if debug:
        return (power,impact_on_ibyj)
    else:
        return power


def totalAEP(turb_diam, turb_coords, power_curve, wind_inst_freq,debug = False):
    """
    -**-THIS FUNCTION SHOULD NOT BE MODIFIED-**-
    ...
    # ###########################################################################################
    #for dubuggin stuff --- added by me
    total_impact_on_ibyj = []
    ...
    for i in range(n_slices_drct):
        for j in range(n_slices_sped): 
            ...
            pwr,impact_on_ibyj  = partAEP(n_turbs, turb_diam, turb_coords, 
                           power_curve, wind_drct, wind_sped,debug = debug)

            #################################################################################
            # added by me
            total_impact_on_ibyj.append(impact_on_ibyj*(wind_inst_freq[i][j]*100))

            farm_pwr[i,j] = pwr
    
    # multiply the respective values with the wind instance probabilities 
    farm_pwr = wind_inst_freq * farm_pwr
    farm_pwr_arr = farm_pwr.copy()
    ...
    AEP = AEP/1e3
    if debug:
        return(AEP,farm_pwr_arr,total_impact_on_ibyj)
    else:
        return (AEP)    
...
def get_eff_wake(coords_file,power_curve_file,wind_data):
    ...
    AEP,farm_pwr,total_impact_on_ibyj = totalAEP(turb_diam, turb_coords, power_curve, wind_inst_freq,debug = True) 
    # print('Total power produced by the wind farm is: ', "%.12f"%(AEP), 'GWh')
    avg_wake_effect = np.mean(np.array(total_impact_on_ibyj),axis = 0)
    return AEP,farm_pwr,avg_wake_effect