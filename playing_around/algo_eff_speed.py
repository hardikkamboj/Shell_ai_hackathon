...
def partAEP(n_turbs, turb_diam, turb_coords, power_curve, wind_drct, wind_sped,debug = False):
    ...
    if debug:
        eff_sped = wind_sped*(1.0 - sped_deficit)
...
def totalAEP(turb_diam, turb_coords, power_curve, wind_inst_freq,debug = False):
    ...
    for i in range(n_slices_drct):
        for j in range(n_slices_sped): 
            ...
            pwr,part_eff_speed  = partAEP(n_turbs, turb_diam, turb_coords, 
                           power_curve, wind_drct, wind_sped,debug = debug)

            #################################################################################
            # added by me
            total_eff_speed.append(part_eff_speed*(wind_inst_freq[i][j]*100))
    ...
    AEP = AEP/1e3
    if debug:
        return(AEP,farm_pwr_arr,total_eff_speed)
    else:
        return (AEP)    
...
def get_eff_speed(coords_file,power_curve_file,wind_data):    
    ...
    AEP,farm_pwr,total_eff_speed = totalAEP(turb_diam, turb_coords, power_curve, wind_inst_freq,debug = True) 
    # print('Total power produced by the wind farm is: ', "%.12f"%(AEP), 'GWh')
    total_eff_speed = np.mean(total_eff_speed,axis=0)
    return AEP,farm_pwr,total_eff_speed
