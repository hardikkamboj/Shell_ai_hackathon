...
def totalAEP(turb_diam, turb_coords, power_curve, wind_inst_freq,debug = False):
    ...
    for i in range(n_slices_drct):
        for j in range(n_slices_sped): 
            ...
            pwr,part_eff_pwr  = partAEP(n_turbs, turb_diam, turb_coords, 
                           power_curve, wind_drct, wind_sped,debug = debug)

            #################################################################################
            # added by me
            total_eff_pwr.append(part_eff_pwr*(wind_inst_freq[i][j]*100))
    ...
    AEP = AEP/1e3
    if debug:
        return(AEP,farm_pwr_arr,total_eff_pwr)
    else:
        return (AEP)    
